...
/**
 * Adapter to bridge existing VCommand classes to Brigadier command system.
 * This allows us to reuse existing command logic while using Paper's Brigadier API.
 */
public class BrigadierCommandAdapter {
    
    /**
     * Creates a Brigadier Command executor that wraps a VCommand.
     * 
     * @param vCommand The VCommand to wrap
     * @param commandName The name of the command
     * @return A Brigadier Command executor
     */
    public static Command<CommandSourceStack> wrapVCommand(
            org.clockworx.vampire.cmd.VCommand vCommand,
            String commandName) {
        return (CommandContext<CommandSourceStack> context) -> {
            ...
            // Execute the wrapped command (onCommand is public, execute is protected)
            boolean result = vCommand.onCommand(sender, command, commandName, args);
